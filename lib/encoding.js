...
const AttestedDevice = {
  preencode (state, a) {
    if (a.type !== ATTESTED_DEVICE) throw new Error('Unexpected type')
...
const AttestedData = {
  preencode (state, a) {
    if (a.type !== ATTESTED_DATA) throw new Error('Unexpected type')
    c.uint.preencode(state, a.version)
...
const DataAttestation = {
  preencode (state, a) {
    c.uint.preencode(state, a.type)
    c.fixed64.preencode(state, a.signature)
  },
  encode (state, a) {
    c.uint.encode(state, a.type)
